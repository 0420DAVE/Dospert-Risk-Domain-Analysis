...
# Ask questions on coursera
write.xlsx(train_freq,file="./temp/train_freq.xlsx",col.names=TRUE,row.names=FALSE,
           sheetName="sheet1")
write.xlsx(test_freq,file="./temp/test_freq.xlsx",col.names=TRUE,row.names=FALSE,
           sheetName="sheet1")
train_freq<-read.xlsx2("./temp/train_freq.xlsx",sheetIndex=1)
test_freq<- read.xlsx2("./temp/test_freq.xlsx",sheetIndex=1)
...
```{r
train_freq_N<-sapply(names(train_freq)[-1],
FUN=function(x){ train_freq[,x]<-as.numeric(train_freq[,x]) })

train_freq_N<-train_freq_N-1
train_freq_N<-data.frame(type=train_raw$type,train_freq_N,row.names=NULL)
#####
test_freq_N<-sapply(names(test_freq)[-1],
FUN=function(x){ test_freq[,x]<-as.numeric(test_freq[,x]) })

test_freq_N<-test_freq_N-1
test_freq_N<-data.frame(type=test_raw$type,test_freq_N,row.names=NULL)

nb<-train(type~.,data=train_freq,method="nb")
pred.nb<-predict(nb,test_freq_N)
pred.nb
test_freq$type
```
